...
public class StudentTester {
	...
	@Test
	public void testIfConstructedCorrectly() throws IOException {
		assertTrue("Failed to create Book and User Lists correctly.",
				LC.getBookCatalog().size() == 50 && LC.getUsers().size() == 30);
	}
	...
	public void testAddBook() {
		...
		assertTrue("Failed to add the information of the books correctly.",
				compareBooks(epicBook1, 51, "My Personal Biography", "G Bonilla", "Classics", LocalDate.of(2023, 9, 15),
						false)
						&& compareBooks(epicBook2, 52, "Coder's Guide to Failing", "G Bonilla", "Adventure",
								LocalDate.of(2023, 9, 15), false));
	...
	public void testCheckOutBook() {
		...
		assertTrue(
				"Failed to checkout a book that ISN'T checked out (id = 19). Should return true"
						+ "or failed to not checkout a book that IS already checked out (id = 20). Should return false",
				success && failure);
	}
	...
	public void testReturnBook() {
		...
		assertTrue(
				"Failed to return a book that IS currently checked out (id = 20) should be true "
						+ "or failed to not return a book that ISN'T checked out (id = 19) should be false",
				success && failure);
	}
	...
	public void testBookToString() {
		assertTrue(
				"Failed to return correct string for book id = 34, expected THE LITTLE PRINCE BY ANTOINE DE SAINT-EXUPERY.",
				this.findBookById(this.LC.getBookCatalog(), 34).toString()
						.equals("THE LITTLE PRINCE BY ANTOINE DE SAINT-EXUPERY"));
	}

	@Test
	public void testBookFees() {
		Book book = this.findBookById(LC.getBookCatalog(), 36);
		assertTrue("Failed to calculate expected fees for book id = 36, expected 389.50 in fees.",
				book.calculateFees() == 389.5);
	}
	...
	private boolean compareBooks(Book actual, int id, String title, String author, String genre, LocalDate ld,
			boolean checkedOut) {
